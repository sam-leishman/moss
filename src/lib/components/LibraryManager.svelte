				...
				>
					...
					>
						<div class="flex flex-col gap-1">
							...
							<span class="text-sm text-gray-600">{library.folder_path}</span>
						</div>
					</button>
					<button 
...
{#if showCreateModal}
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onclick={closeCreateModal}>
		<div class="w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-white rounded-lg shadow-2xl" onclick={(e) => e.stopPropagation()}>
			<div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
				<h3 class="text-lg font-semibold">Create New Library</h3>
			...
			<div class="p-6">
				<div class="mb-4">
					<label for="library-name" class="block mb-2 font-medium text-gray-700">Library Name</label>
					<input
						id="library-name"
						type="text"
				...
				<div class="mb-4">
					<label for="library-path" class="block mb-2 font-medium text-gray-700">Folder Path</label>
					<div class="flex gap-2">
						<input
							...
							class="flex-1 px-3 py-2 text-sm border border-gray-300 rounded-md focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200"
							readonly={showFolderBrowser}
						/>
						<button 
							onclick={() => showFolderBrowser = !showFolderBrowser}
							class="px-4 py-2 text-sm font-medium text-gray-700 transition-colors bg-gray-200 rounded-md hover:bg-gray-300"
						>
							Browse
						</button>
					</div>
				...
				{#if showFolderBrowser}
					<div class="p-4 mt-4 overflow-y-auto border border-gray-200 rounded-md max-h-80">
...
{#if showDeleteConfirm && libraryToDelete}
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onclick={cancelDelete}>
		<div class="w-full max-w-md bg-white rounded-lg shadow-2xl" onclick={(e) => e.stopPropagation()}>
			<div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
				<h3 class="text-lg font-semibold">Delete Library</h3>
